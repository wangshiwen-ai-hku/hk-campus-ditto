...
export function JoinPage({ locale, userId, onUser }: { locale: Locale; userId: string | null; onUser: (id: string) => void; }) {
  ...
  async function verify() {
    const data = await api.verifyCode(email, code, fullName) as { user: StudentProfile };
      ...
      <SectionCard>
        ...
        <div className="grid gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-end gap-4">
            <button className="h-[60px] w-full sm:w-auto rounded-full bg-white px-8 font-black text-black transition-all hover:scale-105 active:scale-95" onClick={requestCode}>{t("join.sendCode")}</button>
            <div className="flex-1 space-y-4 w-full">
                <label className="text-xs font-black uppercase tracking-widest text-white/50">{t("join.verifyCodeLabel")}</label>
                <input className="w-full rounded-2xl border border-white/10 bg-white px-5 py-4 text-black focus:outline-none focus:ring-2 focus:ring-pink-500/50 transition-all" value={code} onChange={(e) => setCode(e.target.value)} />
            </div>
            <button className="h-[60px] w-full sm:w-auto rounded-full border border-white/10 bg-white/5 px-8 font-black text-white hover:bg-white/10 transition-all" onClick={verify}>{t("join.verifyBtn")}</button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-white/5">
            <button className="rounded-full bg-pink-500 px-10 py-5 text-xl font-black text-white shadow-lg transition-all hover:scale-105 active:scale-95" onClick={saveProfile}>{t("join.saveBtn")}</button>
          ...
          {message ? <div className="rounded-2xl bg-green-500/20 border border-green-500/30 p-4 text-center text-sm text-green-300">{message}</div> : null}
        </div>
      </SectionCard>
    </main>
  );
