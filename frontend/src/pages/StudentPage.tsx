...
import { User, Heart, ArrowLeft, X, MapPin, Ruler, Calendar, Globe, Sparkles, Target, ChevronDown, ChevronUp, Flame, Pencil } from "lucide-react";
...
function ProfileTab({ profile, t }: { profile: StudentProfile; t: (k: string, o?: any) => string }) {
  ...
  const hasAttrSignals = dp?.attractionSignals && (dp.attractionSignals.heightAndBuild || dp.attractionSignals.facialFeatures || dp.attractionSignals.energyAndVibe);
      ...
      <div className="space-y-5">
        ...
        <SectionCard className="border-white/10 shadow-2xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            {dp?.ethnicity && <InfoChip icon={<Globe size={14} />} label={t("student.profile.ethnicity")} value={tOpt(t, dp.ethnicity)} />}
        ...
        {(dp?.dateGenders?.length || dp?.datingGoals?.length || dp?.ageRange || dp?.languagePreferences?.length || dp?.matchMode || hasAttrSignals) && (
          <SectionCard className="border-white/10 shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              {dp?.languagePreferences?.length ? (
                <TagRow label={t("student.profile.langPref")} color="bg-white/5 border border-white/10 text-white/60"
                  tags={dp.languagePreferences.map((l) => tOpt(t, l))} />
              ) : null}
              ...
              {hasAttrSignals && (
                <>
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    {dp!.attractionSignals!.facialFeatures && (
                      <div className="rounded-xl bg-white/[0.03] border border-white/5 p-3">
                        <div className="text-[9px] font-black uppercase tracking-wider text-white/25 mb-0.5">{t("student.profile.facialFeatures")}</div>
                        <div className="text-xs text-white/55 font-medium">{dp!.attractionSignals!.facialFeatures}</div>
                      </div>
                    )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        ...
        {dp?.ethnicity && <InfoChip icon={<Globe size={16} />} label={t("student.profile.ethnicity")} value={tOpt(t, dp.ethnicity)} />}
      </div>
            ...
            <div className="rounded-3xl border border-green-400/20 bg-green-400/5 p-6 space-y-4">
              ...
              <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-5">
                ...
                <ul className="space-y-2">
                  {(t("student.matchFlow.giftIdeas", { returnObjects: true }) as string[]).map((idea: string) => (
