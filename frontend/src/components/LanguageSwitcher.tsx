...
export function LanguageSwitcher({ locale, onLocale }: LanguageSwitcherProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 origin-top-right overflow-hidden rounded-2xl border border-white/10 bg-[#0a0f18]/90 backdrop-blur-2xl shadow-2xl animate-in fade-in zoom-in duration-200">
          <div className="p-2 space-y-1">
            {LANGUAGES.map((lang) => (
              ...
              >
                ...
                {locale === lang.id && (
                  <div className="ml-auto h-1.5 w-1.5 rounded-full bg-pink-400 shadow-[0_0_8px_rgba(244,114,182,0.6)]" />
                )}
              </button>
            ))}
          </div>
